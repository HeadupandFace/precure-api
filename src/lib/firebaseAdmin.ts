...
try {
  ...
  if (!serviceAccountKey) {
    throw new Error('FIREBASE_SERVICE_ACCOUNT_KEY is undefined or null.');
  }
...
if (admin.apps.length === 0) {
  try {
    ...
    throw new Error('Failed to initialize Firebase Admin SDK. Check your service account credentials and Firebase project configuration.');
  }
}
